...
public class BattleMaker implements Runnable {
    ...
    private ArrayList<Runnable> enemies;
    ...
    public BattleMaker(AnchorPane mainContainer, ImageView enemyType1, ImageView enemyType2, ArrayList<Runnable> enemies) {
    ...
    public void run() {
        while(true){
            ...
            boolean isActiveCount = enemies.isEmpty();
            System.out.println("IsActive: " + isActiveCount);
...
//            makeFleet("StraferV2", 500 , 5, 6000);
    ...
    private void makeFleet(String enemyType, int startLocation, int countOfEnemies, int spawnTime) {
        ...
        timer.scheduleAtFixedRate(task, 0, spawnTime);
    }
